...
# master_transformer=pd.DataFrame(sentences_master,columns=['Concatenated'])
# master_transformer['tokenize'] = master_transformer['Concatenated'].apply(spacy_tokenizer)

# master_transformer['embeddings'] = master_transformer['tokenize'].apply(model.encode)
...
# tsne_result_master = tsne.fit_transform(embeddings_array)
# tsne_df_master = pd.DataFrame(tsne_result_master, columns=['tsne1', 'tsne2'])
# joblib.dump(tsne_df_master,'tsne_df_master')
tsne_df_master=joblib.load('tsne_df_master')
# fig=px.scatter(x=tsne_df_master['tsne1'],y=tsne_df_master['tsne2'])
# st.plotly_chart(fig)
...
# sentences_sample = sample['Concatenated'].tolist()
# sentences_sample = [s.replace("|", " ") for s in sentences_sample]
# sentences_sample = [s.strip() for s in sentences_sample if s.strip()]
# sample_transformer=pd.DataFrame(sentences_sample,columns=['Concatenated'])
# sample_transformer['tokenize'] = sample_transformer['Concatenated'].apply(spacy_tokenizer)
# sample_transformer['embeddings'] = sample_transformer['tokenize'].apply(model.encode)

# embeddings_array_sample = np.array([np.array(embedding) for embedding in sample_transformer['embeddings']])
# tsne = TSNE(n_components=2)
# tsne_result_sample = tsne.fit_transform(embeddings_array_sample)

# tsne_df_sample = pd.DataFrame(tsne_result_sample, columns=['tsne1', 'tsne2'])
# joblib.dump(tsne_df_sample,'tsne_df_sample')
